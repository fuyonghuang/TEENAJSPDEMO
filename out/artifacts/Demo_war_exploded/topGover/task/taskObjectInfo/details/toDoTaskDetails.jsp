...
<!-- 引入taskObjectInfo SQL -->
<%@ include file="/topGover/task/sqlConstant/taskObjectInfoSql.jsp" %>
<!-- 引入checkInfo SQL -->
<%@ include file="/topGover/task/sqlConstant/checkInfoSql.jsp" %>
...
<%
    ...
    if (!action.equals("insert")) {
        //获取task_info详情
        vPara = new Vector();
        queryObjectInfoSqlStr +=
                OBJECTINFO_AND_TASKINFO_JOIN_QUERY + "AND T.TASK_OBJCT_ID = ? ";
        vPara.add(taskObjectId);
        Record ObjectInfoRecord = sqlManager.getRecords(queryObjectInfoSqlStr, vPara);
...
<form method="post"
      action="/topGover/task/taskObjectInfo/taskObjectInfoSave.jsp?txtAction=claimAction&taskId=<%=record.getString("task_id")%>&taskObjectId=<%=taskObjectId%>"
    ...
    <div class="tablediv">
        ...
        <table class="queryTableResultStyle">
            <tr class="queryTableResultTitleStyle">
                ...
                <td width="10%">表单名称<span id="task_name"></td>

                <td width="10%">表单类型 <span id="start_date"></td>
                <td width="10%">所属单位 <span id="end_date"></td>
                <td>操作</td>
            </tr>
            <%
                //生成表格
